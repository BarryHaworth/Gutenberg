...
# Get a list of files
librivox <- dir(LB_DIR)
lb_json  <- librivox[grepl("json",librivox)]
...
for (i in seq(1,length(lb_xml))){
  ...
  copyright_year  <- as.numeric(xml_data$copyright_year)
  num_sections    <- as.numeric(xml_data$num_sections)
  ...
  author_id         <- as.numeric(xml_data$authors[1]$author$id)
